...
if is_import_relative:
    ...
    from .resources3.http import get_sheets, get_grids, get_delimitations, get_census_tracts
...
class OpenICGC(PluginBase):
    ...
    # Squared (superscript 2) unicode char
    SQUARE_CHAR = "\u00B2"
    ...
    def __init__(self, iface, debug_mode=False):
        ...
        super().__init__(iface, __file__, ini_console=False)

        # Detection of developer enviroment and enable extra opcions in debug_mode
        ...
        # Initialize reference to GeoFinderDialog
        ...
        # Init other variables to empty or dummy values
        self.init_local_resources(empty=True)
        self.init_online_resources(empty=True)
    ...
    def init_static_translations_and_constants(self):
        """ Initialize static translation constants """
        ...
        self.BACKGROUND_MAP_GROUP_NAME = self.tr("Background maps")
        ...
        self.FME_NAMES_DICT = {
            ...
            "topografia-territorial-dwg-object-data": self.tr("Territorial topographic referential object-data"),
            ...
            "topografia-territorial-3d-dwg-object-data": self.tr("Territorial topographic referential 3D object-data"),
            # "topografia-territorial-volum-dwg": self.tr("Territorial topographic referential volume"),
            "referencial-topografic-local-gpkg": self.tr("Local topographic referential"),
            "referencial-topografic-local-dgn": self.tr("Local topographic referential"),
            "referencial-topografic-local-dwg": self.tr("Local topographic referential"),
            "referencial-topografic-local-dwg-object-data": self.tr("Local topographic referential object-data"),
            "referencial-topografic-local-gdb": self.tr("Local topographic referential"),
            "referencial-topografic-local-bim-ifc": self.tr("Local topographic referential BIM"),
            "referencial-topografic-local-3d-gpkg": self.tr("Local topographic referential 3D"),
            "referencial-topografic-local-3d-dgn": self.tr("Local topographic referential 3D"),
            ...
            "entitats-municipals-descentralitzades": self.tr("Decentralized municipal entities"),
            "bsenccen": self.tr("Census tracts"),
            "arees-poblament": self.tr("Population zones"),
            ...
            "mg50m-raster": self.tr("Geological map 1:50,000"),
        ...
        self.FME_ICON_DICT = {
            "ct": "cat_topo5k.png",
            "bm": "cat_topo5k.png",
            "re": "cat_topo5k.png", # referencial-topografic-local
            # "bt" :"cat_topo5k.png",
            "di": "cat_topo5k.png", # divisions-administratives
            "en": "cat_topo5k.png", # entitats-municipals-descentralitzades
            "bs": "cat_topo5k.png", # seccions-censals
            "ar": "cat_topo5k.png", # arees-poblament
            "to": "cat_topo5k.png", # topografia-territorial
            "of": "cat_ortho5k.png",
            "hc": "cat_ortho5k.png",
            "olc10cm": "cat_ortho5k.png",
            "hlc10cm": "cat_ortho5k.png",
            "oi": "cat_ortho5ki.png",
            "hi": "cat_ortho5ki.png",
            "oli10cm": "cat_ortho5ki.png",
        ...
        self.FME_METADATA_DICT = {
            "of": "Current color orthophoto",
            "hc": "Color orthophoto (temporal serie)",
            "olc10cm": "Color local orthophoto",
            "hlc10cm": "Color local orthophoto (temporal serie)",

            "oi": "Current infrared orthophoto",
            "hi": "Infrared orthophoto (temporal serie)",
            "oli10cm": "Infrared local orthophoto",
            "hli10cm": "Infrared local orthophoto (temporal serie)",
            ...
            "topografia-territorial-dwg-object-data": "Territorial topographic referential",
            ...
            "topografia-territorial-3d-dwg-object-data": "Territorial topographic referential",
            # "topografia-territorial-volum-dwg": "Territorial topographic referential",
            "referencial-topografic-local-gpkg": "Local topographic referential",
            "referencial-topografic-local-dgn": "Local topographic referential",
            "referencial-topografic-local-dwg": "Local topographic referential",
            "referencial-topografic-local-dwg-object-data": "Local topographic referential",
            "referencial-topografic-local-gdb": "Local topographic referential",
            "referencial-topografic-local-bim-ifc": "Local topographic referential",
            "referencial-topografic-local-3d-gpkg": "Local topographic referential",
            "referencial-topografic-local-3d-dgn": "Local topographic referential",
            "referencial-topografic-local-3d-dwg": "Local topographic referential",
            "referencial-topografic-local-3d-dwg-object-data": "Local topographic referential",

            "divisions-administratives-shp": "Administrative divisions",
            "divisions-administratives-gpkg": "Administrative divisions",
            "divisions-administratives-dwg": "Administrative divisions",
            "entitats-municipals-descentralitzades": "Decentralized municipal entities",
            "bsenccen": "Census tracts",
            "arees-poblament": "Population zones",
            ...
            "lcosta": "coastline_temporal_serie",
            "ocosta": "coast_orthophoto_temporal_serie",

            "mg50m": "Geological map 1:50,000",
            "mg50m-raster": "Geological map 1:50,000",
        ...
        # Initialize download type descriptions (with translation) and labels
        ...
        # Initialize photo search labels
        self.PHOTOS_GROUP_NAME = self.tr("Photograms")

    def init_static_resources(self):
        """ Initialize static resources (products) """
        # Get Available administrative delimitations, census track and population zones
        self.delimitations_list = [(name, [(scale, "/vsicurl/%s" % scale_file)
            for scale, scale_file in scale_list], style_file)
            for name, scale_list, style_file in get_delimitations()]
        self.decentraliced_municipal_entities_url = "/vsicurl/%s" % get_decentralized_municipal_entities()
        self.census_tracts_url, self.census_tracts_file = get_census_tracts()
        self.census_tracts_url = "/vsicurl/%s" % self.census_tracts_url
        self.population_zones_url = "/vsicurl/%s" % get_population_zones()
    ...
    def unload(self):
        ...
        self.resourcesLoadingFinished.disconnect(self.reload_toolbar)
    ...
    def initGui(self, check_qgis_updates=True, check_icgc_updates=False):
        ...
        # Create toolbar without loaded resources
        self.init_toolbar()

        # Load async resources and update GUI via signal self.resourcesLoadingFinished
        # connected with self.reload_toolbar (can't create QtObjects in a thread...)
        ...
        self.log.info("Partial GUI initialization (%s)" % (t1-t0))
    ...
    def init_resources(self):
        ...
        try:
            ...
            self.log.error("Async initialization: %s" % e)

    def init_local_resources(self, empty=False):
        """ Local resources initialization with real or empty values """
        if empty:
            # Initialize product metadata dictionary with metadata urls
            self.product_metadata_dict = {}
            # Initialize different Catalonia geographic limits
            self.cat_limits_dict = {}
            # Load extra fonts (Fira Sans)
            self.font_id_list = []
            # Detection unit test files
            self.test_list = []

        else:
            self.log.info("Initializing local resources")

            # Initialize product metadata dictionary with metadata urls
            t0 = datetime.datetime.now()
            product_dict_list = []
            product_metadata_pathname = os.path.join(os.path.dirname(__file__), "data", "product_metadata.json")
            if os.path.exists(product_metadata_pathname):
                with open(product_metadata_pathname, 'r') as json_file:
                    product_dict_list = json.load(json_file)
            self.product_metadata_dict = {product_dict['Carpeta']: product_dict['Metadades']
                for product_dict in product_dict_list if product_dict['Metadades']}
            t1 = datetime.datetime.now()
            self.log.info("Load product metadatas: %s (%s)" % (len(self.product_metadata_dict), t1-t0))
            ...
            t1 = datetime.datetime.now()
            self.log.info("Load all Catalonia limits: %s (%s)" % (len(self.cat_limits_dict), t1-t0))
            ...
            if self.debug_mode:
                t0 = datetime.datetime.now()
                self.test_list = self.debug.get_test_names()
                t1 = datetime.datetime.now()
                self.log.info("Load unit tests: %s (%s)" % (len(self.test_list), t1-t0))

    def init_online_resources(self, empty=False, check_qgis_updates=True, check_icgc_updates=False):
        """ Online resources initialization with real or empty values """
        if empty:
            ...
            self.coastline_time_series_list = []
            self.coastline_current_time, self.coastline_current_layer = (None, None)
            self.coast_ortho_url, self.coast_ortho_list = (None, [])
            self.coast_ortho_time_series_list = []
            self.coast_ortho_current_time, self.coast_ortho_current_layer = (None, None)

            # Gets available NDVI files to simulate WMS-T service
            self.ndvi_time_series_list = []
            self.ndvi_current_time = None

            # Gets all ortho data (except satellite)
            self.ortho_wms_url, historic_ortho_list = (None, [])
            self.ortho_color_time_series_list = []
            self.ortho_color_year = None
            self.ortho_infrared_time_series_list = []
            self.ortho_infrared_year = None
            self.ortosuperexp_color_list = []
            self.ortosuperexp_color_year, self.ortosuperexp_color_layer_id, ortosuperexp_color_layer_name = (None, None, None)
            self.ortosuperexp_infrared_list = []
            self.ortosuperexp_infrared_year, self.ortosuperexp_infrared_layer_id, ortosuperexp_infrared_layer_name = (None, None, None)
            self.ortoxpres_color_list = []
            self.ortoxpres_color_year, self.ortoxpres_color_layer_id, ortoxpres_color_layer_name = (None, None, None)
            self.ortoxpres_infrared_list = []
            self.ortoxpres_infrared_year, self.ortoxpres_infrared_layer_id, ortoxpres_infrared_layer_name = (None, None, None)
            self.lidar_ortho_color_time_series_list = []
            self.lidar_ortho_color_year = None
            self.lidar_ortho_infrared_time_series_list = []
            self.lidar_ortho_infrared_year = None
            self.ortho_local_wms_url, historic_ortho_local_list = (None, [])
            self.ortho_color_local_time_series_list = []
            self.ortho_color_local_year, self.ortho_color_local_current_layer = (None, None)
            self.ortho_infrared_local_time_series_list = []
            self.ortho_infrared_local_year, self.ortho_infrared_local_current_layer = (None, None)

            # Gets anaglyph photograms. Last year can not have full photograms coverage, we select previous year as default
            self.photolib_wms_url = PHOTOLIB_WMS
            _photolib_time_series_list, self.photolib_current_time = ([], None)
            ...
            self.new_qgis_plugin_version = None
            self.new_plugin_version = None

        else:
            self.log.info("Initializing online resources")
            ...
            self.coastline_time_series_list = [(coastline_date_tag, coastline_layer) for coastline_layer, _coastline_name,  coastline_date_tag in self.coastline_list]
            self.coastline_current_time, self.coastline_current_layer = self.coastline_time_series_list[-1] if self.coastline_time_series_list else ("", "")
            self.coast_ortho_url, self.coast_ortho_list = get_coast_orthos()
            self.coast_ortho_time_series_list = [(coast_ortho_date_tag, coast_ortho_layer) for coast_ortho_layer, _coast_ortho_name, _coast_ortho_color_type, coast_ortho_date_tag in self.coast_ortho_list]
            self.coast_ortho_current_time, self.coast_ortho_current_layer = self.coast_ortho_time_series_list[-1] if self.coast_ortho_time_series_list else ("", "")

            # Gets available NDVI files to simulate WMS-T service
            self.ndvi_time_series_list = [(time_year, "/vsicurl/%s" % url) for time_year, url in get_ndvis()]
            self.ndvi_current_time = self.ndvi_time_series_list[-1][0] if self.ndvi_time_series_list else None

            # Gets all ortho data (except satellite)
            self.ortho_wms_url, historic_ortho_list = get_full_ortho()
            self.ortho_color_time_series_list = [(str(year), layer_id) for layer_id, layer_name, ortho_type, color, year in historic_ortho_list if ortho_type == "ortofoto" and color != "irc"]
            self.ortho_color_year = self.ortho_color_time_series_list[-1][0] if self.ortho_color_time_series_list else None
            self.ortho_infrared_time_series_list = [(str(year), layer_id) for layer_id, layer_name, ortho_type, color, year in historic_ortho_list if ortho_type == "ortofoto" and color == "irc"]
            self.ortho_infrared_year = self.ortho_infrared_time_series_list[-1][0] if self.ortho_infrared_time_series_list else None
            self.ortosuperexp_color_list = [(str(year), layer_id, layer_name) for layer_id, layer_name, ortho_type, color, year in historic_ortho_list if ortho_type == "superexpedita" and color != "irc"]
            self.ortosuperexp_color_year, self.ortosuperexp_color_layer_id, ortosuperexp_color_layer_name = self.ortosuperexp_color_list[-1] if self.ortosuperexp_color_list else (None, None, None)
            self.ortosuperexp_infrared_list = [(str(year), layer_id, layer_name) for layer_id, layer_name, ortho_type, color, year in historic_ortho_list if ortho_type == "superexpedita" and color == "irc"]
            self.ortosuperexp_infrared_year, self.ortosuperexp_infrared_layer_id, ortosuperexp_infrared_layer_name = self.ortosuperexp_infrared_list[-1] if self.ortosuperexp_infrared_list else (None, None, None)
            self.ortoxpres_color_list = [(str(year), layer_id, layer_name) for layer_id, layer_name, ortho_type, color, year in historic_ortho_list if ortho_type == "ortoxpres" and color != "irc"]
            self.ortoxpres_color_year, self.ortoxpres_color_layer_id, ortoxpres_color_layer_name = self.ortoxpres_color_list[-1] if self.ortoxpres_color_list else (None, None, None)
            self.ortoxpres_infrared_list = [(str(year), layer_id, layer_name) for layer_id, layer_name, ortho_type, color, year in historic_ortho_list if ortho_type == "ortoxpres" and color == "irc"]
            self.ortoxpres_infrared_year, self.ortoxpres_infrared_layer_id, ortoxpres_infrared_layer_name = self.ortoxpres_infrared_list[-1] if self.ortoxpres_infrared_list else (None, None, None)
            historic_lidar_ortho_list = get_lidar_ortho()
            self.lidar_ortho_color_time_series_list = [(year, "/vsicurl/%s" % url) for year, url, color in historic_lidar_ortho_list if color == "rgb"]
            self.lidar_ortho_color_year = self.lidar_ortho_color_time_series_list[-1][0] if self.lidar_ortho_color_time_series_list else None
            self.lidar_ortho_infrared_time_series_list = [(year, "/vsicurl/%s" % url) for year, url, color in historic_lidar_ortho_list if color == "irc"]
            self.lidar_ortho_infrared_year = self.lidar_ortho_infrared_time_series_list[0][0] if self.lidar_ortho_infrared_time_series_list else None
            self.ortho_local_wms_url, historic_ortho_local_list = get_full_local_ortho()
            self.ortho_color_local_time_series_list = [(str(year), layer_id) for layer_id, layer_name, color, year in historic_ortho_local_list if color == "rgb"]
            self.ortho_color_local_year, self.ortho_color_local_current_layer = self.ortho_color_local_time_series_list[-1] if self.ortho_color_local_time_series_list else (None, None)
            self.ortho_infrared_local_time_series_list = [(str(year), layer_id) for layer_id, layer_name, color, year in historic_ortho_local_list if color == "irc"]
            self.ortho_infrared_local_year, self.ortho_infrared_local_current_layer = self.ortho_infrared_local_time_series_list[-1] if self.ortho_infrared_local_time_series_list else (None, None)

            # Gets anaglyph photograms. Last year can not have full photograms coverage, we select previous year as default
            t0 = datetime.datetime.now()
            self.photolib_wms_url = PHOTOLIB_WMS
            _photolib_time_series_list, self.photolib_current_time = self.layers.get_wms_t_time_series(self.photolib_wms_url, "photo_central")
            self.photolib_current_time = str(int(self.photolib_current_time) - 1) if self.photolib_current_time else self.photolib_current_time
            t1 = datetime.datetime.now()
            self.log.info("Photolib current time: %s (%s)" % (self.photolib_current_time, t1-t0))

            # Gets available download source data
            self.log.info("Initializing FME downloads")
            self.fme_services_list = get_services()

            # Check plugin update
            t0 = datetime.datetime.now()
            self.new_icgc_plugin_version = self.check_plugin_update() if check_icgc_updates else None
            self.new_qgis_plugin_version = self.metadata.get_qgis_new_version_available() if check_qgis_updates and not self.lite else None
            self.new_plugin_version = self.new_qgis_plugin_version or self.new_icgc_plugin_version
            t1 = datetime.datetime.now()
            self.log.info("Plugin updates: %s (%s)" % (self.new_plugin_version, t1-t0))

    def reload_toolbar(self):
        """ Reload QGIS toolbar for plugin when resources are loaded """
        # Remove previous toolbar without resources loaded
    ...
    def init_toolbar(self):
        """ Creates QGIS toolbar for plugin """

        # Create combobox to do searches
        ...
        self.toolbar = self.gui.configure_toolbar(self.tr("Open ICGC Toolbar") + (" lite" if self.lite else ""), [
            ...
            (self.tr("Background maps"),
                ...
                (self.tr("Topographic map"), None, "cat_topo1m", [
                    (self.tr("Topographic map 1:250,000"),
                        ...
                        "https://geoserveis.icgc.cat/servei/catalunya/topografia-250000/wms",
                        ["topografia-250000"], ["default"], "image/png", 25831, self.request_referrer_param,
                    ...
                    (self.tr("Topographic map 1:1,000,000"),
                        lambda _checked: self.layers.add_wms_layer(self.tr("Topographic map 1:1,000,000"),
                        "https://geoserveis.icgc.cat/servei/catalunya/topografia-1000000/wms",
                        ["topografia-1000000"], ["default"], "image/png", 25831, self.request_referrer_param,
                        self.BACKGROUND_MAP_GROUP_NAME, only_one_map_on_group=False, set_current=True),
                        "cat_topo1m.png",
                ...
                (self.tr("Administrative divisions"), None, "cat_vector.png", self.enable_http_files, [
                    (self.tr("Administrative divisions (vector pyramid)"),
                        lambda _checked: self.layers.add_remote_layer_definition_file(
                    ...
                    (self.tr("Administrative divisions (raster pyramid)"),
                        lambda _checked: self.layers.add_wms_layer(self.tr("Administrative divisions (raster pyramid)"),
                            "https://geoserveis.icgc.cat/servei/catalunya/divisions-administratives/wms",
                            ...
                            ["default"], "image/png", 25831, self.request_referrer_param, self.BACKGROUND_MAP_GROUP_NAME, only_one_map_on_group=False, set_current=True),
                        "cat_vector.png",
                        self.manage_metadata_button("Administrative divisions"), True),
                    "---",
                    ] + [
                        (self.HTTP_NAMES_DICT.get(name, name),
                        (lambda _checked, name=name, scale_list=scale_list, style_file=style_file: self.layers.add_vector_layer(self.HTTP_NAMES_DICT.get(name, name), scale_list[0][1], group_name=self.BACKGROUND_MAP_GROUP_NAME, only_one_map_on_group=False, set_current=True, style_file=style_file) if len(scale_list) == 1 else None),
                        "cat_vector.png", ([
                            ("%s 1:%s" % (self.HTTP_NAMES_DICT.get(name, name), self.format_scale(scale)),
                                lambda _checked, name=name, scale=scale, url=url, style_file=style_file: self.layers.add_vector_layer("%s 1:%s" % (self.HTTP_NAMES_DICT.get(name, name), self.format_scale(scale)), url, group_name=self.BACKGROUND_MAP_GROUP_NAME, only_one_map_on_group=False, set_current=True, style_file=style_file),
                                "cat_vector.png",
                                self.manage_metadata_button("Administrative divisions"), True)
                            for scale, url in scale_list] if len(scale_list) > 1
                            else self.manage_metadata_button("Administrative divisions")),
                            len(scale_list) == 1)
                        for name, scale_list, style_file in self.delimitations_list
                    ] + [
                    "---",
                    (self.tr("Population zones"),
                        lambda _checked: self.layers.add_vector_layer(self.tr("Population zones"),
                            self.population_zones_url,
                            group_name=self.BACKGROUND_MAP_GROUP_NAME, only_one_map_on_group=False,
                            set_current=True),
                        "cat_vector.png", self.manage_metadata_button("Population zones"), True),
                    (self.tr("Decentralized municipal entities"),
                        lambda _checked: self.layers.add_vector_layer(self.tr("Decentralized municipal entities"),
                            self.decentraliced_municipal_entities_url,
                            group_name=self.BACKGROUND_MAP_GROUP_NAME, only_one_map_on_group=False,
                            set_current=True),
                        "cat_vector.png", self.manage_metadata_button("Decentralized municipal entities"), True),
                    (self.tr("Census tracts"),
                        lambda _checked: self.layers.add_vector_layer(self.tr("Census tracts"),
                            self.census_tracts_url,
                            group_name=self.BACKGROUND_MAP_GROUP_NAME, only_one_map_on_group=False,
                            set_current=True, style_file=self.census_tracts_file),
                        "cat_vector.png", self.manage_metadata_button("Census tracts"), True),
                    ]),
                (self.tr("Cartographic series"), None, "sheets.png", self.enable_http_files, [(
                    self.tr("%s serie") % sheet_name,
                    lambda _checked, sheet_name=sheet_name, sheet_url=sheet_url: self.layers.add_vector_layer(
                        self.tr("%s serie") % sheet_name, sheet_url,
                        group_name=self.BACKGROUND_MAP_GROUP_NAME, only_one_map_on_group=False, set_current=True,
                        style_file=sheet_style_file),
                    "sheets.png", self.enable_http_files,
                    self.manage_metadata_button("Cartographic series"), True
                    ) for sheet_name, sheet_url, sheet_style_file in self.sheets_list
                    ] + [
                    "---"
                    ] + [(
                        self.tr("%s grid") % grid_name,
                        lambda _checked, grid_name=grid_name, grid_url=grid_url: self.layers.add_vector_layer(
                            self.tr("%s grid") % grid_name, grid_url,
                            group_name=self.BACKGROUND_MAP_GROUP_NAME, only_one_map_on_group=False, set_current=True,
                            style_file=grid_style_file),
                        "sheets.png", self.enable_http_files,
                ...
                (self.tr("Geological map"), None, "cat_geo250k.png", [
                    (self.tr("Geological map 1:50,000"),
                        lambda _checked: self.layers.add_wms_layer(self.tr("Geological map 1:50,000"),
                        "https://geoserveis.icgc.cat/servei/catalunya/geologia-territorial/wms",
                        ["geologia-territorial-50000-geologic"], ["default"], "image/png", 25831, self.request_referrer_param,
                        self.BACKGROUND_MAP_GROUP_NAME, only_one_map_on_group=False, set_current=True, resampling_bilinear=True),
                        "cat_geo50k.png",
                        self.manage_metadata_button("Geological map 1:50,000"), True),
                    (self.tr("Geological map 1:250,000"),
                        lambda _checked: self.layers.add_wms_layer(self.tr("Geological map 1:250,000"),
                        "https://geoserveis.icgc.cat/servei/catalunya/geologia-territorial/wms",
                        ["geologia-territorial-250000-geologic"], ["default"], "image/png", 25831, self.request_referrer_param,
                        self.BACKGROUND_MAP_GROUP_NAME, only_one_map_on_group=False, set_current=True, resampling_bilinear=True),
                        "cat_geo250k.png",
                        self.manage_metadata_button("Geological map 1:250,000"), True),
                    ]),
                (self.tr("Land cover map (temporal serie)"),
                    lambda _checked: self.add_wms_t_layer(self.tr("[TS] Land cover map"),
                    "https://geoserveis.icgc.cat/servei/catalunya/cobertes-sol/wms",
                    None, None, "default", "image/png", None, r"cobertes_(.+)", 25831,
                    self.request_referrer_param + "&bgcolor=0x000000",
                    self.BACKGROUND_MAP_GROUP_NAME, only_one_map_on_group=False, set_current=True),
                    "cat_landcover.png",
                    ...
                    self.tr("Coastline"), None, "cat_coast.png", [
                        (self.tr("Coastline (temporal serie)"),
                            lambda _checked: self.add_wms_t_layer(
                                ...
                                self.coastline_url, self.coastline_current_layer, self.coastline_current_time,
                                "default", "image/png", self.coastline_time_series_list, None,
                            ...
                            "cat_coast.png", len(self.coastline_time_series_list) > 0,
                            self.manage_metadata_button("coastline_temporal_serie"), True),
                        "---"
                        ] + [(
                            "%s %s" % (self.tr("Coastline"), coastline_name),
                            lambda _checked, coastline_name=coastline_name, coastline_url=self.coastline_url, coastline_layer=coastline_layer:
                                self.layers.add_wms_layer(
                                    "%s %s" % (self.tr("Coastline"), coastline_name),
                                    coastline_url, [coastline_layer], ["default"], "image/png", 25831,
                                    self.request_referrer_param,
                                    group_name=self.BACKGROUND_MAP_GROUP_NAME, group_pos=0, only_one_map_on_group=False,
                                    set_current=True, resampling_bilinear=True),
                            "cat_coast.png", True,
                            # self.manage_metadata_button("coastline_" + coastline_date_tag), True
                            self.manage_metadata_button("coastline_temporal_serie"), True
                    ...
                    (self.tr("Coast orthophoto"), None, "cat_coast.png", [
                        (self.tr("Coast orthophoto (temporal serie)"),
                            lambda _checked: self.add_wms_t_layer(
                                ...
                                self.coast_ortho_url, self.coast_ortho_current_layer, self.coast_ortho_current_time,
                                "default", "image/png",
                                self.coast_ortho_time_series_list, None,
                                25831, self.request_referrer_param,
                                group_name=self.BACKGROUND_MAP_GROUP_NAME, group_pos=0, only_one_map_on_group=False,
                                set_current=True, resampling_bilinear=True),
                            "cat_coast.png", len(self.coast_ortho_time_series_list) > 0,
                            self.manage_metadata_button("coast_orthophoto_temporal_serie"), True),
                        "---"
                        ] + [
                        ("%s %s" % (self.tr("Coast orthophoto"), coast_ortho_name),
                            lambda _checked, coast_ortho_name=coast_ortho_name, coast_ortho_url=self.coast_ortho_url, coast_ortho_layer=coast_ortho_layer:
                                self.layers.add_wms_layer("%s %s" % (self.tr("Coast orthophoto"), coast_ortho_name),
                                    coast_ortho_url, [coast_ortho_layer], ["default"], "image/png", 25831,
                                    self.request_referrer_param,
                                    group_name=self.BACKGROUND_MAP_GROUP_NAME, group_pos=0, only_one_map_on_group=False,
                                    set_current=True, resampling_bilinear=True),
                            "cat_coast.png", True,
                            # self.manage_metadata_button("coast-orthophoto-" + coast_ortho_date_tag), True
                            self.manage_metadata_button("coast_orthophoto_temporal_serie"), True)
                        for coast_ortho_layer, coast_ortho_name, _coast_ortho_color_type, coast_ortho_date_tag in reversed(self.coast_ortho_list)
                    ])
                ]),
                "---",
                ...
                (self.tr("Current gray orthophoto") + " (%s)" % self.ortho_color_year,
                    ...
                    self.ortho_wms_url,
                    ["ortofoto_gris_vigent"], ["default"], "image/png", 25831, self.request_referrer_param + "&bgcolor=0x000000",
                    self.BACKGROUND_MAP_GROUP_NAME, only_one_map_on_group=False, set_current=True),
                    "cat_ortho5kbw.png",
                ...
                (self.tr("Centered photogram"), None, "photo.png", [
                    (self.tr("Centered photogram (annual serie)"),
                        lambda _checked: self.add_wms_t_layer(self.tr("[AS] Centered photogram"),
                ...
                "---",
                    ...
                    (self.tr("Others"), None, "cat_cmstandard.png", [
                        (self.tr("ContextMaps standard map"),
                            lambda _checked: self.layers.add_wms_layer(self.tr("ContextMaps standard map"),
                            "https://geoserveis.icgc.cat/servei/catalunya/contextmaps/wms",
                            ["contextmaps-mapa-estandard"], [""], "image/jpeg", 25831, self.request_referrer_param,
                            self.BACKGROUND_MAP_GROUP_NAME, only_one_map_on_group=False, set_current=True),
                            "cat_cmstandard.png",
                            self.manage_metadata_button("ContextMaps standard map"), True),
                        (self.tr("ContextMaps gray map"),
                            lambda _checked: self.layers.add_wms_layer(self.tr("ContextMaps gray map"),
                            "https://geoserveis.icgc.cat/servei/catalunya/contextmaps/wms",
                            ["contextmaps-mapa-base-gris"], [""], "image/jpeg", 25831, self.request_referrer_param,
                            self.BACKGROUND_MAP_GROUP_NAME, only_one_map_on_group=False, set_current=True),
                            "cat_cmgray.png",
                            self.manage_metadata_button("ContextMaps gray map"), True),
                        (self.tr("ContextMaps simplified gray map"),
                            lambda _checked: self.layers.add_wms_layer(self.tr("ContextMaps simplified gray map"),
                            "https://geoserveis.icgc.cat/servei/catalunya/contextmaps/wms",
                            ["contextmaps-mapa-base-gris-simplificat"], [""], "image/jpeg", 25831, self.request_referrer_param,
                            self.BACKGROUND_MAP_GROUP_NAME, only_one_map_on_group=False, set_current=True),
                            "cat_cmgray.png",
                            self.manage_metadata_button("ContextMaps simplified gray map"), True),
                        (self.tr("ContextMaps hybrid orthophoto"),
                            lambda _checked: self.layers.add_wms_layer(self.tr("ContextMaps hybrid orthophoto"),
                            "https://geoserveis.icgc.cat/servei/catalunya/contextmaps/wms",
                            ["contextmaps-orto-hibrida"], [""], "image/jpeg", 25831, self.request_referrer_param,
                            self.BACKGROUND_MAP_GROUP_NAME, only_one_map_on_group=False, set_current=True),
                            "cat_cmortho.png",
                            self.manage_metadata_button("ContextMaps hybrid orthophoto"), True),
                        "---",
                        (self.tr("Instamaps pyramid"),
                            lambda: self.layers.add_wms_layer(self.tr("Instamaps pyramid"),
                            "https://tilemaps.icgc.cat/mapfactory/service",
                            ["osm_suau"], ["default"], "image/png", 25831, '',
                            self.BACKGROUND_MAP_GROUP_NAME, only_one_map_on_group=False, set_current=True),
                            "cat_topo5k.png"),
                        ]),
                    "---",
                    (self.tr("Spain"), None, "spain_topo.png", [
                        (self.tr("IGN topographic"),
                            lambda: self.layers.add_wms_layer(self.tr("IGN topographic"),
                            "http://www.ign.es/wms-inspire/mapa-raster", ["mtn_rasterizado"], ["default"], "image/png", 25830, '',
                            self.BACKGROUND_MAP_GROUP_NAME, only_one_map_on_group=False, set_current=True),
                            "spain_topo.png"),
                        "---",
                        (self.tr("PNOA orthophoto"),
                            lambda: self.layers.add_wms_layer(self.tr("PNOA orthophoto"),
                            "http://www.ign.es/wms-inspire/pnoa-ma", ["OI.OrthoimageCoverage"], ["default"], "image/png", 25830, '',
                            self.BACKGROUND_MAP_GROUP_NAME, only_one_map_on_group=False, set_current=True),
                            "spain_orto.png"),
                        "---",
                        (self.tr("Cadastral registry"),
                            lambda: self.layers.add_wms_layer(self.tr("Cadastral registry"),
                            "http://ovc.catastro.meh.es/Cartografia/WMS/ServidorWMS.aspx", ["Catastro"], ["default"], "image/png", 25831, '',
                            self.BACKGROUND_MAP_GROUP_NAME, only_one_map_on_group=False, set_current=True),
                            "spain_cadastral.png"),
                        ]),
                    (self.tr('Andorra'), None, "andorra_topo50k.png", [
                        (self.tr("Andorra topographic 1:50,000 2020"),
                            lambda: self.layers.add_wms_layer(self.tr("Andorra topographic 1:50,000 2020"),
                            "https://www.ideandorra.ad/Serveis/wmscarto50kraster_2020/wms",
                            ["mta50m2020geotif"], [], "image/png",  27563, '',
                            self.BACKGROUND_MAP_GROUP_NAME, only_one_map_on_group=False, set_current=True),
                            "andorra_topo50k.png"),
                        "---",
                        (self.tr("Andorra orthophoto 1:5,000 2012"),
                            lambda: self.layers.add_wms_layer(self.tr("Andorra orthophoto 1:5,000 2012"),
                            "https://www.ideandorra.ad/Serveis/wmsorto2012/wms", ["orto2012"], [], "image/png",  27563, '',
                            self.BACKGROUND_MAP_GROUP_NAME, only_one_map_on_group=False, set_current=True),
                            "andorra_orto2003.png"),
                        ]),
                    (self.tr("France"), None, "france_topo.png", [
                        (self.tr("France topographic"),
                            lambda: self.layers.add_wms_layer(self.tr("France topographic"),
                            "http://mapsref.brgm.fr/wxs/refcom-brgm/refign", ["FONDS_SCAN"], [], "image/png", 32631, '',
                            self.BACKGROUND_MAP_GROUP_NAME, only_one_map_on_group=False, set_current=True),
                            "france_topo.png"),
                        "---",
                        (self.tr("France orthophoto 20cm"),
                            lambda: self.layers.add_wms_layer(self.tr("France orthophoto 20cm"),
                            "https://data.geopf.fr/annexes/ressources/wms-r/ortho.xml",
                            ["HR.ORTHOIMAGERY.ORTHOPHOTOS"], ["normal"], "image/png", 32631, '',
                            self.BACKGROUND_MAP_GROUP_NAME, only_one_map_on_group=False, set_current=True, ignore_get_map_url=False),
                            "france_ortho.png"),
                        ]),
                    (self.tr("World"), None, "world.png", [
                        (self.tr("OpenStreetMap"),
                            lambda: self.layers.add_wms_layer(self.tr("OpenStreetMap"),
                            "http://ows.terrestris.de/osm/service", ["OSM-WMS"], [], "image/png", 4326, '',
                            self.BACKGROUND_MAP_GROUP_NAME, only_one_map_on_group=False, set_current=True),
                            "world.png"),
                        (self.tr("NASA blue marble"),
                            lambda: self.layers.add_wms_layer(self.tr("NASA blue marble"),
                            "http://geoserver.webservice-energy.org/geoserver/ows",
                            ["gn:bluemarble-2048"], [], "image/png", 4326, '',
                            self.BACKGROUND_MAP_GROUP_NAME, only_one_map_on_group=False, set_current=True),
                            "world.png"),
                         ]),
                    "---",
                ] if self.extra_countries or self.debug_mode else []) + [
                (self.tr("Delete background maps"), lambda _checked: self.legend.empty_group_by_name(self.BACKGROUND_MAP_GROUP_NAME),
            ...
            (self.tr("Time series"),
                ...
                (self.tr("Search photograms"), (self.enable_search_photos, self.pair_photo_search_checks),
                    "search.png", self.enable_photo_search, True, "photo_search", [
                ...
                (self.tr("Download tool"), self.enable_last_download, "download_area.png", True, True, "download", [
                    ...
                    "---"
                    ] + self.get_download_menu(self.fme_services_list, raster_not_vector=True) + [
                    "---",
                    (self.tr("Save map as PDF"), lambda: self.save_map("A4H", "", self.tr("Save map as PDF")),
                        "pdf.png", True, False, "save_pdf"),
                    (self.tr("Save location map as PDF"),
                        lambda: self.save_map("A4V", self.tr(" (location)"), self.tr("Save location map as PDF")),
                        "pdf.png", True, False, "save_location_pdf"),
                ])
            ] if not self.lite else []) + [
            (self.tr("Paint styles for selected layers"), None, "style.png", [
                ...
                (self.tr("Height highlighting"),
                    lambda _checked, dtm_url=self.height_highlighting_url: self.add_height_highlighting_layer(
                        self.tr("Height highlighting"), dtm_url, style_file="ressaltat_alçades.qml",
                        group_name=self.BACKGROUND_MAP_GROUP_NAME),
                    "cat_shadows.png", self.enable_http_files and self.height_highlighting_url),
                (self.tr("Shading DTM layer"), self.shading_dtm, "cat_shadows.png"),
                "---",
            ...
            (self.tr("Help"), self.show_help, "help.png", [
                ...
                self.tr("Update\n available: v%s") % self.new_qgis_plugin_version,
                (self.tr("Download plugin"),
                    lambda _checked, v=self.new_qgis_plugin_version: self.download_plugin_update(v, UpdateType.plugin_manager),
                    "new.png"),
            ]) + ([] if not self.new_icgc_plugin_version or self.lite else [
                self.tr("Update\n available: v%s") % self.new_icgc_plugin_version,
                (self.tr("Download plugin"),
                    lambda _checked, v=self.new_icgc_plugin_version: self.download_plugin_update(v, UpdateType.icgc_web),
                    "new_icgc.png"),
        ...
        if not self.lite:
            if self.debug_mode or self.test_list:
        ...
        # Get a reference to any actions to an easy update
    ...
    def get_download_menu(self, fme_services_list, raster_not_vector=None, nested_download_submenu=True):
                ...
                if id:
                    ...
                    if gsd:
                        ...
                        product_submenu.append((
                            ...
                            self.FME_ICON_DICT.get(id, None) or self.FME_ICON_DICT.get(prefix_id, None),
        ...
        # Prepare "all in one" download submenu
            ...
            last_part_id = id.split()[-1]
            download_submenu = [(
                product_file_label_pattern % (
                    name, raster_label if self.is_raster_file(filename)
                    else vector_label if self.is_vector_file(filename)
                    else points_label if self.is_points_file(filename)
                    else "",
                    os.path.splitext(filename)[1][1:]),
                (lambda _dummy, id=id, name=name, min_side=min_side, max_query_area=max_query_area,
                    min_px_side=min_px_side, max_px_area=max_px_area, time_list=time_list,
                    download_list=download_list, filename=filename, limits=limits,
                    url_ref_or_wms_tuple=url_ref_or_wms_tuple:
                    self.enable_download_subscene(id, name, min_side, max_query_area,
                        min_px_side, max_px_area, time_list,
                        download_list, filename, limits, url_ref_or_wms_tuple),
                    self.pair_download_checks),
                self.FME_ICON_DICT.get(id, None) or self.FME_ICON_DICT.get(id[:2], None),
                enabled, True, id, # Indiquem: actiu, checkable i un id d'acció
                self.manage_metadata_button(self.FME_METADATA_DICT.get(id, None)
                    or self.FME_METADATA_DICT.get(prefix_id, None)
                    or (self.FME_METADATA_DICT[first_part_id] + " " + last_part_id
                    if self.FME_METADATA_DICT.get(first_part_id, None)
                    else None)),
                True
                ) if id else "---"
                for id, name, min_side, max_query_area, min_px_side, max_px_area, filename, limits, vectorial_not_raster, url_pattern, url_ref_or_wms_tuple, enabled in fme_extra_services_list
            ]
    ...
    def enable_last_download(self, checked=False):
        ...
        self.legend.set_group_visible_by_name(self.BACKGROUND_MAP_GROUP_NAME)
    ...
    def find(self, user_text, x=None, y=None):
        ...
        # We create the GeoFinderDialog if it is necessary
        if not self.geofinder_dialog:
    ...
    def is_vector_file(self, pathname):
        return self.is_file_type(pathname, ["shp", "dgn", "dwg", "gpkg", "shp-zip", "gdb-zip"])

    def is_vector_extension(self, ext):
        return self.is_extension(ext, ["shp", "dgn", "dwg", "gpkg", "shp-zip", "gdb-zip"])
    ...
    def enable_download_subscene(self, data_type, name, min_side, max_download_area, min_px_side, max_px_area, time_list, download_list, filename, limits, url_ref_or_wms_tuple, gsd_dict={}):
        ...
        if gsd_dict:
            ...
            data_dict = {year: {gsd: {"download": {description: id
                ...
                if operation_code in download_list}, "max_area": gsd_max_download_area}
                for (gsd, (_data_type, _name, _min_side, gsd_max_download_area, _min_px_side, _max_px_area, _time_list, download_list, _filename, _limits, _url_ref_or_wms_tuple, _enabled)) in gsd_dict.items()
            ...
            if not time_list:
                time_list = [None]
            data_dict = {year: {None: {"download": download_type_dict, "max_area": max_download_area}} for year in time_list}
        # Open dialog
        if not self.download_dialog:
            self.download_dialog = DownloadDialog(data_dict, self.translation.get_qgis_language())
        else:
        ...
        if url_ref_or_wms_tuple:
            ...
            if is_historic_ortho:
                ...
                name += " %d" % time_code
            if is_coast_ortho:
                ref_url, ref_layer, ref_style, ref_format, symbol_file = url_ref_or_wms_tuple
                ref_layer = get_coast_ortho_ref(time_code)
                url_ref_or_wms_tuple = (ref_url, ref_layer, ref_style, ref_format, symbol_file) if ref_layer else None
                name += " %s" % time_code
            if is_local_ortho:
                ref_file, symbol_file = url_ref_or_wms_tuple
                color_not_irc = data_type.startswith("hlc") or data_type.startswith("olc")
    ...
    def download_map_area(self, geo, data_type, min_side, max_download_area, min_px_side, max_px_area, gsd, time_code, download_operation_code, local_filename, limits="cat_simple"):
        ...
        if is_photo:
            ...
            # If is historic ortho download, add extra param in_filename to download
            rgb_not_irc = data_type.startswith("hlc")
            ortho_code = get_historic_local_ortho_code(rgb_not_irc, gsd, time_code)
            extra_params = [ortho_code]
            filename = "%s_%s" % (os.path.splitext(filename)[0], time_code)
        elif is_coastline:
            # If is time coast download, add extra param with filename
            filename = get_coastline_filename_dict()[time_code]
        ...
        if self.download_type not in ["dt_cat", "dt_all"]:
            ...
            if not geo:
                return
        is_polygon = (type(geo) is QgsGeometry)
        is_area = (type(geo) is QgsRectangle and not geo.isEmpty())
        ...
        try:
            if is_compressed:
                if is_unsupported_format:
                    ...
                    if not download_layer:
                        ...
                        download_layer = self.layers.add_vector_files([os.path.join(download_folder, local_filename)],
                            group_name=self.DOWNLOAD_GROUP_NAME, group_pos=0, only_one_visible_map_on_group=False,
                            regex_styles_list=self.fme_regex_styles_list)
            elif is_unsupported_format or is_slow_format:
                # With an unsupported format we only download file
                local_filename = self.layers.download_remote_file(url, local_filename, download_folder=None,
                    title=self.tr("Downloading ..."), cancel_button_text=self.tr("Cancel"),
                    time_info=self.tr("Elapsed %s"))
            elif is_raster:
                ...
                if is_photo and download_operation_code == "tot":
                    ...
                    reference_system_ok = self.layers.download_remote_file(xml_file_url, xml_pathname,
                        download_folder, unzip=True, title=self.tr("Downloading ..."),
                ...
                if geometry_ok and reference_system_ok:
                    ...
                    download_layer = self.layers.add_remote_raster_file(url, local_filename,
                        ...
                        color_default_expansion=color_expansion,
                        resampling_bilinear=True, title=self.tr("Downloading ..."),
                        cancel_button_text=self.tr("Cancel"), time_info=self.tr("Elapsed %s"))
            elif is_points:
                ...
                download_layer = self.layers.add_remote_vector_file(url, local_filename,
                    group_name=self.DOWNLOAD_GROUP_NAME, group_pos=0, only_one_visible_map_on_group=False,
                    regex_styles_list=self.fme_regex_styles_list, title=self.tr("Downloading ..."),
                    cancel_button_text=self.tr("Cancel"), time_info=self.tr("Elapsed %s"))
        except Exception as e:
            error = str(e)
        ...
        if download_layer and data_filter and (not is_points or self.can_filter_point_cloud):
            ...
            self.layers.set_filter(download_layer, data_filter)
        ...
        t1 = datetime.datetime.now()
        self.log.info("Download time: %s" % (t1-t0))
        ...
        if is_slow_format:
            ...
            if not self.can_filter_point_cloud and is_points and download_layer and data_filter:
                QMessageBox.warning(None, title, self.tr(
                    "The current version of QGIS does not allow filtering data from point cloud files, "
    ...
    def download_get_geometry(self, geo, download_epsg, min_side, max_download_area, min_px_side, max_px_area, gsd, limits, default_point_buffer=50, max_aspect_ratio=FME_MAX_ASPECT_RATIO):
        ...
        is_polygon = (type(geo) is QgsGeometry)
        is_point = (type(geo) is QgsRectangle and geo.isEmpty())
        is_area = (type(geo) is QgsRectangle and not geo.isEmpty())
    ...
    def load_ref_layer(self, url_ref_or_wfs_or_wms_tuple, name):
        ...
        if layer:
            ...
            with WaitCursor():
                ...
                if len(url_ref_or_wfs_or_wms_tuple) == 5: # Load WMS layer + Style file
                    wms_url, wms_layer, wms_style, wms_format, style_file = url_ref_or_wfs_or_wms_tuple
                    ...
                    layer = self.layers.add_wms_layer(layer_name, wms_url, [wms_layer], [wms_style] if wms_style else None, wms_format,
                        None, self.request_referrer_param + "&bgcolor=0x000000", self.BACKGROUND_MAP_GROUP_NAME, 0, only_one_visible_map_on_group=False)
                    self.layers.load_style(layer, style_file)

                elif len(url_ref_or_wfs_or_wms_tuple) == 4: # Load WMS layer
                    ...
                    layer = self.layers.add_wfs_layer(layer_name, wfs_url, [wfs_layer],
                        extra_tags=self.request_referrer_param, group_name=self.BACKGROUND_MAP_GROUP_NAME, group_pos=0, style_file=style_file, only_one_visible_map_on_group=False)
                elif len(url_ref_or_wfs_or_wms_tuple) == 2: # Load HTTP
                    url_ref, style_file = url_ref_or_wfs_or_wms_tuple
                    url_ref = "/vsicurl/" + url_ref
                    is_raster = self.is_raster_file(url_ref) or url_ref.lower().find("orto-territorial") >= 0
                    if is_raster:
                        # Load raster layer from URL
                        layer = self.layers.add_raster_layer(layer_name, url_ref, self.BACKGROUND_MAP_GROUP_NAME, 0, transparency=70, style_file=style_file, only_one_visible_map_on_group=False)
                    else:
                        # Load vector layer from URL
                        layer = self.layers.add_vector_layer(layer_name, url_ref, self.BACKGROUND_MAP_GROUP_NAME, 0, transparency=70, style_file=style_file, only_one_visible_map_on_group=False)
...
Update your version of qgis if possible.""") % Qgis.QGIS_VERSION,
            self.tr("QGIS version warnings"), LogInfoDialog.mode_warning, width=1000, height=250)
    ...
    def shading_dtm(self, checked=False):
        ...
        layer = self.iface.mapCanvas().currentLayer()
        if type(layer) is not QgsRasterLayer or layer.bandCount() > 1:
            QMessageBox.information(self.iface.mainWindow(), title,
                self.tr('You must select a DTM layer'))
            return
        self.layers.load_style(layer, "ombrejat.qml")
        self.layers.refresh(layer)
        # Show shading info
        QMessageBox.information(self.iface.mainWindow(), title,
    ...
    def search_photos(self, x=None, y=None, name=None, photolib_wfs=PHOTOLIB_WFS,
            date_field="flight_date", max_features=1000):
        """ Search photos in photo library by selected point or photo name """
        photo_search_label = self.tr("Photo query: %s")
        title = photo_search_label.replace(": %s", "")
        epsg = None
        ...
        with WaitCursor():
            ...
            if x and y:
                ...
                if municipality:
                    layer_name = photo_search_label % municipality
                else:
                    if x >= 100 and y >= 100:
                        layer_name = photo_search_label % self.tr("Coord %s %s") % ("%.2f" % x, "%.2f" % y)
                    else:
                        layer_name = photo_search_label % self.tr("Coord %s %s") % (x, y)
            ...
            if name and len(name) > 7: # at least flight code and wildcard ...
                layer_name = photo_search_label % name
            ...
            search_photos_year_list = sorted(list(set([(f[date_field].date().year() if type(f[date_field]) is QDateTime
    ...
    def photo_preview(self, photo_name, rectified=False, stereo=False, only_one=True, photolib_wms=PHOTOLIB_WMS):
        ...
        photo_label = self.tr("Photo: %s") % photo_name
        if photo_layer:
    ...
    def report_issue_form(self, new_plugin_version=None, error=""):
        ...
        form_id = "V5S73HJB-UO3rUQwD03qJkuaYVuCv_VIuHYZCM2bVrZUOFZRTEpMNUpMRTBGQ1BTRTVYRFlFNDdZUSQlQCN0PWcu" # pragma: allowlist secret
        ...
        # Open issues form
        url = base_url % (form_id, self.translation.get_qgis_language(), coord_text, environment_text, error)
        self.show_url(url)
    ...
    def force_project_epsg_when_empty(self, layer, epsg):
        """ Set project EPSG if empty project """
        # Wait pending events
        QApplication.processEvents()
        # Only force epsg with empty project (only 1 new layer)
        layer_count = len(QgsProject.instance().mapLayers())
        if layer_count <= 1:
            self.project.set_epsg(epsg)
        return layer
